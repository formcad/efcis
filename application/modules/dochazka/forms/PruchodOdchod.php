...
/**
 * Formulář pro přidání nebo změnu odchodu v docházce
 */
class Dochazka_Form_PruchodOdchod extends ZendX_JQuery_Form
{
    /**
     * Typy průchodů
     * @var array
    ...
    {        
        ...
        /**** Element - typ průchodu (odchod) *********************************/
        
        $pruchodOdchod = new Zend_Form_Element_Select('typ');
        $pruchodOdchod->setLabel('Typ průchodu')
                ->setRequired(true)
                ->addValidator(new Zend_Validate_NotEmpty);
        ...
        if (!empty(self::$typyPruchodu)) {
            foreach (self::$typyPruchodu as $typ) {
                ...
                $pruchodOdchod->addMultiOption($typ["id"],$typ["nazev"]);                
            }            
        }        
        ...
        $this->addElements(array($datumSmeny,$datumPruchodu,$cas,$pruchodOdchod,$submit));    
    }
}